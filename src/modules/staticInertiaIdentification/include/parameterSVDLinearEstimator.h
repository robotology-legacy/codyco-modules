...
#ifndef _PARAMETER_SVD_LINEAR_ESTIMATOR
#define _PARAMETER_SVD_LINEAR_ESTIMATOR
...
/**
 * Class for performing online estimation of parameters
 ...
 *
 * As we want to modify the set of selected "Essential" parameters at runtime
 * we don't recursivly calculate \f$ (\phi^\top \mathbf{W} \phi)^{-1} \f$, but we
 * store \f$ (\phi^\top \mathbf{W} \phi) \f$ to iterativly calculate the essential
 * parameters.
 */
class parameterSVDLinearEstimator
{
protected:
    ...
    Eigen::MatrixXd                 A;      ///< Inverse covariance matrix (i.e. A).
    Eigen::JacobiSVD<Eigen::MatrixXd> svd_A;///< SVD of A
...
public:
    ...
    parameterSVDLinearEstimator(unsigned int nParam = 1, unsigned int nOutputs = 1, double lambda = 1.0);
    ...
    inline void setParamSize(unsigned int size, double lambda = 1.0) { n = size; resizeAllVariables(lambda); }
    ...
    * @param size The desired codomain size. */
    ...
    void getSingularValues(Eigen::VectorXd & singular_values);

    /** Returns the V matrix */
    void getSingularVectors(Eigen::MatrixXd &  singular_vectors);
