...
int main (int argc, char * argv[])
{
    ...
    rf.setDefaultContext("wholeBodyReach");  //when no parameters are given to the module this is the default context
    rf.configure(argc,argv);

    if (rf.check("help"))
    {
        cout<< "Possible parameters"                                                                                                                                          << endl << endl;
        cout<< "\t--context          :Where to find an user defined .ini file within $ICUB_ROOT/app e.g. /wholeBodyReachCtrl/conf"                                   <<endl;
