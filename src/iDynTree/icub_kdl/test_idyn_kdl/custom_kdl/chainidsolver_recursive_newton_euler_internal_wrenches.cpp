...
#include "chainidsolver_recursive_newton_euler_internal_wrenches.hpp"
#include <kdl/frames_io.hpp>

namespace KDL{
    
    ChainIdSolver_RNE_IW::ChainIdSolver_RNE_IW(const Chain& chain_,Vector grav):
    ...
    int ChainIdSolver_RNE_IW::CartToJnt_and_internal_wrenches(const JntArray &q, const JntArray &q_dot, const JntArray &q_dotdot, const Wrenches& f_ext,JntArray &torques,Wrenches& f_int)
    {
        ...
        for(int i=ns-1;i>=0;i--){
            if(chain.getSegment(i).getJoint().getType()!=Joint::None)
                ...
                //std::cout << "Obtained " << dot(S[i],f[i]) << "from dot product of " << S[i] << " and " << f[i] << std::endl;
            if(i!=0) {
                //DEBUG
                /*
                std::cout << "~~~~~~~~~~~Backward recursion step for i = " << i << std::endl;
                std::cout << "f[i-1] before" << std::endl;
