...
class staticInertiaIdentification_IDLServer_start : public yarp::os::Portable {
public:
  bool _return;
...
class staticInertiaIdentification_IDLServer_stop : public yarp::os::Portable {
public:
  bool _return;
  virtual bool write(yarp::os::ConnectionWriter& connection) {
    yarp::os::idl::WireWriter writer(connection);
...
bool staticInertiaIdentification_IDLServer::start() {
  bool _return = false;
  staticInertiaIdentification_IDLServer_start helper;
  if (!yarp().canWrite()) {
...
bool staticInertiaIdentification_IDLServer::stop() {
  bool _return = false;
  staticInertiaIdentification_IDLServer_stop helper;
  if (!yarp().canWrite()) {
    fprintf(stderr,"Missing server method '%s'?\n","bool staticInertiaIdentification_IDLServer::stop()");
  }
  bool ok = yarp().write(helper,helper);
...
bool staticInertiaIdentification_IDLServer::read(yarp::os::ConnectionReader& connection) {
  yarp::os::idl::WireReader reader(connection);
  reader.expectAccept();
...
std::vector<std::string> staticInertiaIdentification_IDLServer::help(const std::string& functionName) {
  ...
  if(showAll) {
    ...
    if (functionName=="start") {
      ...
      helpString.push_back("Start the estimation. ");
